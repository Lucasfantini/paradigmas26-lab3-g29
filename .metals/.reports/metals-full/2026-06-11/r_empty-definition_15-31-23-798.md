error id: file://<WORKSPACE>/src/main/scala/Main.scala:formatTypeStats.
file://<WORKSPACE>/src/main/scala/Main.scala
empty definition using pc, found symbol in pc: formatTypeStats.
empty definition using semanticdb
empty definition using fallback
non-local guesses:
	 -Formatters.formatTypeStats.
	 -Formatters.formatTypeStats#
	 -Formatters.formatTypeStats().
	 -scala/Predef.Formatters.formatTypeStats.
	 -scala/Predef.Formatters.formatTypeStats#
	 -scala/Predef.Formatters.formatTypeStats().
offset: 4080
...
object Main {
  def main(args: Array[String]): Unit = {
    ...
    *
    * Map-Reduce para contar entidades:
    */

    // El diccionario se carga en el driver
    val dictionary = Dictionary.loadAll(cmdArgs.entitiesDir)

    // Detectamos entidades como en el skeleton original
    val allEntities = filteredPosts.flatMap { post =>
      val combinedText = post.title + " " + post.selftext
      Analyzer.detectEntities(combinedText, dictionary)
    }

    // Convertimos las entidades a un RDD
    val entitiesRDD = sc.parallelize(allEntities)
    ...
    // Traemos el resultado al driver
    val entityCounts = entityCountsRDD.collect().toMap

    // Estadísticas por tipo (igual que antes)
    val typeStats = Analyzer.countByType(allEntities)

    println(Formatters.formatTyp@@eStats(typeStats))
    println()
